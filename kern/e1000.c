...
struct e1000_tdh *tdh;//发送头指针
struct e1000_tdt *tdt;//发送尾指针
struct e1000_tx_desc tx_desc_array[TXDESCS];//发送描述符队列
char tx_buffer_array[TXDESCS][TX_PKT_SIZE];//缓冲区

struct e1000_rdh *rdh;//接受队列的头指针
...
//通过DD标志位就能知道某个描述符是否能被回收，满
	...
	uint32_t next = (current + 1) % TXDESCS;//尾指针后移
	tdt->tdt = next;
	return 0;
}
//52:54:00:12是MAC地址的 low-order 32位，34:56是high-order 16位。
static void
...
e1000_receive_init()
{
	uint32_t *rdbal = (uint32_t *)E1000REG(E1000_RDBAL);//RDAL物理地址
...
e1000_receive(void *addr, size_t *len)
{
	...
	if(!rx_desc_array[next].status & E1000_RXD_STAT_DD)//检查DD位
	{
